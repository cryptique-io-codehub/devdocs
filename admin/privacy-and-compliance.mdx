...
Trodo is designed with privacy in mind. This guide covers data handling practices, compliance features, and how to configure privacy settings for your team.
...
Trodo uses IP addresses for geolocation but does not store them:
...
Wallet addresses (e.g. `0x1234...abcd`) are stored as-is and are **pseudonymous by nature** — they identify a wallet, not a named individual, unless the user explicitly links additional identity information (such as an email address via `Trodo.identify()`).

Important considerations:

- Blockchain transaction data is **publicly available**. Anyone can look up a wallet's history on a block explorer. Trodo aggregates this public data; it does not expose anything that isn't already on-chain.
- Trodo does **not** use heuristics or clustering to deanonymize wallets. A wallet address is only linked to a user profile when the End-User explicitly connects their wallet.
...
Trodo can be loaded conditionally based on user consent:
...
OneTrust.OnConsentChanged(function(e) {
  if (OnetrustActiveGroups.includes('C0002')) {
    // Performance cookies consented - load Trodo
    loadTrodo();
  }
});
```
...
All Trodo data is stored and processed on **Google Cloud Platform (GCP)** in the **us-central1** region (Iowa, United States).
...
Enterprise customers may request EU-only data residency as part of a custom agreement. Contact [privacy@trodo.ai](mailto:privacy@trodo.ai) to discuss data residency requirements.
...
Trodo's infrastructure runs on Google Cloud Platform, which holds SOC 2 Type II and ISO 27001 certifications at the infrastructure level. Trodo's own SOC 2 Type II audit is currently in progress. When completed, the report will be available to enterprise customers under NDA.

## Compliance

### GDPR

Trodo supports GDPR compliance:
...
- Update privacy policy to mention Trodo
...
Trodo uses first-party cookies for:
...
For customers who require a formal Data Processing Agreement (DPA) — including those operating under GDPR — Trodo provides:
...
To request any of these documents, email [privacy@trodo.ai](mailto:privacy@trodo.ai) with the subject line **"DPA Request"**. We aim to respond within 2 business days.
...
We use Trodo for product analytics. Trodo collects:
...
For more information, see Trodo's privacy policy at 
https://trodo.ai/privacy
```
...
<AccordionGroup>
  ...
  <Accordion title="How to implement consent">
    Use conditional loading - only load Trodo after user grants consent. See "Consent-Based Tracking" section above.
  </Accordion>
</AccordionGroup>
