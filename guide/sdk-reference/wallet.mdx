...
## Cryptique.connectWallet()

Trigger the wallet connection flow programmatically.
...
`void` - The wallet address is captured automatically when the user connects.
...
```jsx
// React component
<button onClick={() => Cryptique.connectWallet()}>
  Connect Wallet
</button>
```
...
Manually associate a wallet address with the current user. Use this when you already have the wallet address from your own connection flow.
...
There is no explicit `Cryptique.disconnectWallet()` method. Disconnection is handled automatically:
...
<Tabs>
  ...
  <Tab title="Web3Modal">
    ...
    modal.subscribeProvider(({ address, isConnected }) => {
      if (isConnected && address) {
        Cryptique.walletAddress(address);
        Cryptique.track('Wallet Connected', { address });
      }
    });
    ```
  </Tab>
...
window.ethereum?.on('chainChanged', (chainId) => {
  Cryptique.track('Chain Switched', {
