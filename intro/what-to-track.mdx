---
title: "What to Track"
description: "A guide to planning your Trodo implementation for maximum insight"
---
...
Trodo unifies three types of data into complete user profiles:
...
- `wallet_connected` (custom — call `Trodo.track()` after connect, alongside `Trodo.walletAddress()`)
...
Auto-events are **off** until you set `auto-events="true"` (or call `Trodo.enableAutoEvents()`). The SDK emits many named events (clicks, scroll, forms, errors, performance, `page_summary`, etc.). Enable the bundle, then **disable** what you do not want:
...
See [Auto-Events](/data-in/auto-events) for every event name and the JavaScript API. For wallets, use `Trodo.walletAddress()` and custom `track()` events rather than expecting `wallet_connect` auto events.
...
// Errors and friction
Trodo.track('transaction_failed', {
...
4. Trodo automatically indexes all transactions
...
<Tabs>
  <Tab title="✅ Good">
    ...
    Trodo.track('nft_minted', { ... });
    Trodo.track('liquidity_added', { ... });
    Trodo.track('onboarding_step_completed', { ... });
    ```
  </Tab>
  <Tab title="❌ Avoid">
    ```javascript
    Trodo.track('SwapCompleted', { ... });     // PascalCase
    Trodo.track('swap-completed', { ... });    // kebab-case
    Trodo.track('Swap Completed', { ... });    // Spaces
    Trodo.track('swap', { ... });              // Too vague
    ```
  </Tab>
</Tabs>
