...
<Update label="Developer documentation" icon="book">

- New [Groups](/data-in/sdk-reference/groups) SDK reference: `set_group`, `add_group`, `remove_group`, `get_group`, event enrichment, and `identify()` hydration.
- [Auto-Events](/data-in/auto-events) rewritten to match the current SDK: enable/disable defaults, full event name catalog, JavaScript API, SPA path handling, and notes on `page_summary`, errors, and performance events.
...
<Update label="Auto Events — Major SDK Overhaul" icon="bolt">
...
- **Renamed events**: `click` → `element_click`, `scroll` → `page_scroll` with richer payloads.
...
- **JS Runtime API**: `Cryptique.disableAutoEvent()`, `Cryptique.enableAutoEvent()`, `Cryptique.setAutoEventsDisabledEvents()`, `Cryptique.getAutoEventsConfig()`.
...
<Update label="Cohorts in Analytics" icon="chart-pie">
...
<Update label="User & cohort filters" icon="filter">
