...
- `set_group` → **replaces** the user's membership for a given key. Events fired after the switch are attributed to the new group only — not the old one.
- Historical events (up to 90 days) are backfilled automatically when membership changes.
...
**Replaces** the user's membership for `groupKey` with one ID (or an array of IDs). Any previously set IDs for that key are removed.

Use this when the user **switches** their active context (e.g. switching from one team to another).

```javascript
// User switches to a different team — old team is replaced
...
<ParamField path="groupId" type="string | string[]" required>
  Single ID or array of IDs. Replaces the entire previous list for that key.
</ParamField>

**What happens internally:**
- Updates `localStorage` immediately (optimistic).
- Sends to the server, which removes the user from the old group's member list and adds them to the new group.
- Backfills the last **24 hours** of the user's events: replaces the old group's ID with the new group's ID on those events.
- Events older than 24 hours keep their original attribution (they were fired while the user was in the old group — correct behaviour).
...
- Backfills the last **90 days** of the user's events: appends the new group's ID to those events without removing any existing group attributions.
...
- Strips the group's ID from the last **24 hours** of the user's events so they no longer appear in that group's timeline.
