---
title: "Data Model"
description: "Understanding Trodo's unified data model for Web3 analytics"
---

## Overview

Trodo's data model is designed to solve the unique challenges of Web3 analytics by unifying three types of data into cohesive user profiles:
...
- **Custom events**: Tracked via `Trodo.track()`
...
Users can belong to **groups** (companies, workspaces, teams) under a string **group key** and one or more **group IDs**. Membership is updated with `Trodo.set_group` / `add_group` / `remove_group`. Properties on the group itself (not the person) are updated via `Trodo.get_group(key, id).set(...)`. Group fields are merged into event `custom_properties` when events are sent. See [Groups](/data-in/sdk-reference/groups).
...
The power of Trodo is in automatically linking these data types:
...
```javascript
Trodo.walletAddress("0x1234..."); // First wallet
Trodo.walletAddress("0xabcd..."); // Second wallet
...
Trodo supports these property data types:
